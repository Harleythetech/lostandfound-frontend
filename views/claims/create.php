                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="d-flex mb-4 pb-4 border-bottom">
                            ...
                            if (!empty($claimItem['images'][0])) {
                                ...
                                $imgPath = is_array($img) ? ($img['url'] ?? $img['image_path'] ?? $img['file_name'] ?? '') : $img;
                                $itemImage = normalizeImageUrl($imgPath);
                            } elseif (!empty($claimItem['primary_image'])) {
                                $itemImage = normalizeImageUrl(is_array($claimItem['primary_image']) ? ($claimItem['primary_image']['url'] ?? $claimItem['primary_image']['file_name'] ?? '') : $claimItem['primary_image']);
                            }
                            ?>
                            <?php if ($itemImage): ?>
                                <img src="<?= htmlspecialchars($itemImage) ?>" class="rounded me-3"
                            ...
                            <div>
                                ...
                                <p class="text-muted mb-1 small"><i
                                        class="bi bi-geo-alt me-1"></i><?= htmlspecialchars($claimItem['location']['name'] ?? 'N/A') ?>
                                </p>
                                <p class="text-muted mb-0 small"><i class="bi bi-calendar me-1"></i>Found on
                                    <?= formatDate($claimItem['found_date'] ?? $claimItem['date_found'] ?? $claimItem['created_at']) ?>
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="alert alert-info py-2 mb-4 small">
                            ...
                            <strong>Important:</strong> Provide accurate details about the item to verify ownership.
                            Include any distinctive features only the true owner would know.
                        </div>
                        ...
                        <form action="<?= APP_URL ?>/claims" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-4">
                                <label for="description" class="form-label">Describe the item in detail <span
                                        class="text-danger">*</span></label>
                                <textarea class="form-control" id="description" name="description" rows="4"
                                    placeholder="Describe the item including brand, color, size, contents, any marks or damage, etc."
                                    minlength="20" required></textarea>
                                <small class="text-muted">Be as specific as possible (min. 20 characters)</small>
                            </div>

                            <div class="mb-4">
                                <label for="proof_details" class="form-label">Proof of Ownership Details <span
                                        class="text-danger">*</span></label>
                                <textarea class="form-control" id="proof_details" name="proof_details" rows="4"
                                    placeholder="Serial numbers, unique marks, scratches, stickers, engravings, exact contents..."
                                    minlength="20" required></textarea>
                                <small class="text-muted">Details only the true owner would know (min. 20
                                    characters)</small>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <small class="text-muted">Receipts, photos with the item, or other proof (max 5 images,
                                    5MB each)</small>
                            </div>
                            ...
                            <div class="d-grid gap-2">
                                <button type="submit" class="btn btn-warning btn-lg"><i
                                        class="bi bi-send me-2"></i>Submit Claim</button>
                                <a href="<?= APP_URL ?>/found-items/<?= $claimItem['id'] ?>"
