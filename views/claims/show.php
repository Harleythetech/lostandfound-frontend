...
function normalizeClaimViewImageUrl($imgPath) {
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="d-flex justify-content-between position-relative mt-3">
                                <div class="progress position-absolute" style="height: 4px; top: 12px; left: 12%; right: 12%; z-index: 0;">
                                    <?php
                                ...
                                <div class="text-center" style="z-index: 1;">
                                    <div class="rounded-circle <?= $isComplete ? 'bg-success' : 'bg-secondary' ?> text-white d-flex align-items-center justify-content-center mx-auto mb-1" style="width: 28px; height: 28px; font-size: 12px;">
                        ...
                        <div class="alert alert-danger py-2">
                            <i class="bi bi-x-circle me-1"></i><strong>Rejected:</strong> <?= htmlspecialchars($claim['rejection_reason'] ?? 'No reason provided.') ?>
                        </div>
                        <?php endif; ?>

                        <?php if ($status === 'cancelled'): ?>
                        <div class="alert alert-secondary py-2">
                            <i class="bi bi-x-circle me-1"></i><strong>Cancelled:</strong> This claim has been cancelled.
                        </div>
                        <?php endif; ?>
                        ...
                        <div class="alert alert-success py-2">
                            ...
                            <strong>Date:</strong> <?= date('F d, Y \a\t g:i A', strtotime($claim['pickup_scheduled'])) ?>
                            <?php if (!empty($claim['storage_location'])): ?><br><strong>Location:</strong> <?= htmlspecialchars($claim['storage_location']) ?><?php endif; ?>
                        </div>
                        <?php elseif ($status === 'approved'): ?>
                        <div class="alert alert-info py-2">
                            <i class="bi bi-info-circle me-1"></i>Your claim has been approved! Please wait for the pickup schedule.
                        </div>
                        <?php endif; ?>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="row g-2">
                                ...
                                <?php 
                                $proofUrl = normalizeClaimViewImageUrl($image['url'] ?? '');
                                ?>
                                <?php if (!empty($proofUrl)): ?>
                                <div class="col-3 col-md-2">
                                    <a href="<?= htmlspecialchars($proofUrl) ?>" target="_blank">
                                        <img src="<?= htmlspecialchars($proofUrl) ?>" class="img-thumbnail" style="width: 100%; height: 60px; object-fit: cover;">
                                    </a>
                                    <?php if (!empty($image['description'])): ?>
                                    <small class="d-block text-muted text-truncate"><?= htmlspecialchars($image['description']) ?></small>
                                    <?php endif; ?>
                                </div>
                                <?php endif; ?>
                                <?php endforeach; ?>
                        ...
                        <div class="text-muted small">
                            <i class="bi bi-clock me-1"></i>Submitted <?= date('M d, Y g:i A', strtotime($claim['created_at'])) ?>
                            <?php if (!empty($claim['verified_at'])): ?>
                            <br><i class="bi bi-check-circle me-1"></i>Approved <?= date('M d, Y g:i A', strtotime($claim['verified_at'])) ?>
            ...
            <div class="col-lg-4">
                ...
                <div class="card border mb-4">
                    ...
                    if (!empty($itemImages)) {
                        ...
                        if ($primaryImg) {
                            $itemImageUrl = normalizeClaimViewImageUrl($primaryImg['url'] ?? $primaryImg['file_name'] ?? '');
                        }
                    } elseif (!empty($claim['item_primary_image'])) {
                        $itemImageUrl = normalizeClaimViewImageUrl($claim['item_primary_image']);
                    }
                    
                    if (empty($itemImageUrl)) {
                        $itemImageUrl = APP_URL . '/assets/img/no-image.svg';
                    }
                    
                    $itemId = $claim['item_id'] ?? $claim['found_item_id'] ?? '';
                    ?>
                    <img src="<?= htmlspecialchars($itemImageUrl) ?>" class="card-img-top" style="height: 150px; object-fit: cover;">
                    <div class="card-body p-3">
                        <h6 class="mb-1"><?= htmlspecialchars($claim['item_title'] ?? 'Unknown Item') ?></h6>
                        <p class="text-muted small mb-2 text-truncate"><?= htmlspecialchars($claim['item_description'] ?? '') ?></p>
                        <?php if (!empty($claim['category_name'])): ?>
                        <span class="badge bg-light text-dark me-1"><i class="bi bi-tag me-1"></i><?= htmlspecialchars($claim['category_name']) ?></span>
                        <?php endif; ?>
                        <?php if (!empty($claim['found_location'])): ?>
                        <span class="badge bg-light text-dark"><i class="bi bi-geo-alt me-1"></i><?= htmlspecialchars($claim['found_location']) ?></span>
                        <?php endif; ?>
                        <div class="mt-3">
                            <a href="<?= APP_URL ?>/found-items/<?= $itemId ?>" class="btn btn-outline-primary btn-sm w-100">
                ...
                <div class="card border mb-4">
                    <div class="card-header bg-white py-2"><h6 class="mb-0"><i class="bi bi-gear me-2"></i>Actions</h6></div>
                    <div class="card-body">
                        <form action="<?= APP_URL ?>/claims/<?= $claim['id'] ?>/cancel" method="POST" onsubmit="return confirm('Cancel this claim?');">
                            <button type="submit" class="btn btn-outline-danger w-100"><i class="bi bi-x-circle me-1"></i>Cancel Claim</button>
                        </form>
                    </div>
                </div>
                <?php endif; ?>
                ...
                <div class="card border">
                    <div class="card-header bg-white py-2"><h6 class="mb-0"><i class="bi bi-person-lines-fill me-2"></i>Contact</h6></div>
                    <div class="card-body">
                        ...
                        <p class="mb-0 small text-muted"><i class="bi bi-person-badge me-1"></i><?= htmlspecialchars($claim['finder_school_id']) ?></p>
                        <?php endif; ?>
                        <?php elseif ($isFinder): ?>
                        ...
                        <p class="mb-0"><i class="bi bi-telephone me-1"></i><?= htmlspecialchars($claim['claimant_contact']) ?></p>
                        <?php endif; ?>
                        <?php if (!empty($claim['claimant_email'])): ?>
                        <p class="mb-0"><i class="bi bi-envelope me-1"></i><?= htmlspecialchars($claim['claimant_email']) ?></p>
                        <?php endif; ?>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="card border mt-4">
                    <div class="card-header bg-white py-2"><h6 class="mb-0"><i class="bi bi-geo-alt me-2"></i>Pickup Location</h6></div>
                    <div class="card-body">
