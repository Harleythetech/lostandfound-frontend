        ...
        <ul class="nav nav-pills mb-4 gap-2">
            <li class="nav-item">
                <a class="nav-link <?= !isset($_GET['status']) || $_GET['status'] === '' ? 'active' : '' ?>"
                    href="<?= APP_URL ?>/claims">All</a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= ($_GET['status'] ?? '') === 'pending' ? 'active' : '' ?>"
                    href="<?= APP_URL ?>/claims?status=pending">Pending</a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= ($_GET['status'] ?? '') === 'verified' ? 'active' : '' ?>"
                    href="<?= APP_URL ?>/claims?status=verified">Verified</a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= ($_GET['status'] ?? '') === 'scheduled' ? 'active' : '' ?>"
                    href="<?= APP_URL ?>/claims?status=scheduled">Scheduled</a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= ($_GET['status'] ?? '') === 'completed' ? 'active' : '' ?>"
                    href="<?= APP_URL ?>/claims?status=completed">Completed</a>
            </li>
        </ul>
                    ...
                    <div class="col-md-6">
                        <div class="card border h-100">
                            <div class="card-header bg-white d-flex justify-content-between align-items-center py-2">
                                <span
                                    class="badge <?= getStatusBadgeClass($claim['status']) ?>"><?= ucfirst($claim['status']) ?></span>
                                <small class="text-muted">Claim #<?= $claim['id'] ?></small>
                            </div>
                            <div class="card-body p-3">
                                <div class="d-flex mb-3">
                                    <?php if (!empty($claim['found_item']['images'][0])): ?>
                                        <img src="<?= htmlspecialchars(normalizeImageUrl($claim['found_item']['images'][0] ?? '')) ?>"
                                    ...
                                    <div>
                                        <h6 class="mb-1">
                                            <?= htmlspecialchars($claim['found_item']['title'] ?? $claim['found_item']['item_name'] ?? 'Unknown Item') ?>
                                        </h6>
                                        <p class="text-muted small mb-0"><i
                                                class="bi bi-geo-alt me-1"></i><?= htmlspecialchars($claim['found_item']['location']['name'] ?? 'Unknown') ?>
                                        </p>
                                        <p class="text-muted small mb-0"><i
                                                class="bi bi-calendar me-1"></i><?= formatDate($claim['created_at']) ?></p>
                                    </div>
                                </div>

                                <?php if ($claim['status'] === 'scheduled' && !empty($claim['pickup_date'])): ?>
                                    <div class="alert alert-info py-2 mb-0 small">
                                        <i class="bi bi-calendar-check me-1"></i><strong>Pickup:</strong>
                            ...
                            <div class="card-footer bg-white border-0 pt-0">
                                <div class="d-flex gap-2">
                                    <a href="<?= APP_URL ?>/found-items/<?= $claim['found_item_id'] ?>"
                    ...
                    <ul class="pagination justify-content-center">
                        <?php for ($i = 1; $i <= $pagination['totalPages']; $i++): ?>
                            <li class="page-item <?= $i == ($pagination['currentPage'] ?? 1) ? 'active' : '' ?>">
                                <a class="page-link"
                                    href="?page=<?= $i ?><?= isset($_GET['status']) ? '&status=' . $_GET['status'] : '' ?>"><?= $i ?></a>
                            </li>
                        <?php endfor; ?>
                    </ul>
                </nav>
