                ...
                <form action="<?= APP_URL ?>/items" method="GET" class="row g-3">
                    ...
                    <div class="col-md-3">
                        <select class="form-select" name="category">
                            ...
                            <option value="documents" <?= ($_GET['category'] ?? '') === 'documents' ? 'selected' : '' ?>>
                                Documents</option>
                            <option value="accessories" <?= ($_GET['category'] ?? '') === 'accessories' ? 'selected' : '' ?>>Accessories</option>
                            <option value="clothing" <?= ($_GET['category'] ?? '') === 'clothing' ? 'selected' : '' ?>>
                                Clothing</option>
                            <option value="keys" <?= ($_GET['category'] ?? '') === 'keys' ? 'selected' : '' ?>>Keys
                            </option>
                            <option value="bags" <?= ($_GET['category'] ?? '') === 'bags' ? 'selected' : '' ?>>Bags
                            </option>
                            <option value="others" <?= ($_GET['category'] ?? '') === 'others' ? 'selected' : '' ?>>Others
                            </option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        <select class="form-select" name="status">
                            ...
                            <option value="found" <?= ($_GET['status'] ?? '') === 'found' ? 'selected' : '' ?>>Found
                            </option>
                    ...
                    <div class="col-md-2">
                        <div class="d-flex gap-2">
                            ...
                            <a href="<?= APP_URL ?>/items" class="btn btn-outline-secondary"><i
                    ...
                    <div class="col-md-6 col-lg-4">
                        <a href="<?= APP_URL ?>/items/<?= $item['id'] ?>"
                            ...
                            <div class="position-relative">
                                ...
                                <?php endif; ?>
                                <span
                                    class="badge position-absolute top-0 end-0 m-2 <?= ($item['status'] ?? '') === 'lost' ? 'bg-danger' : (($item['status'] ?? '') === 'found' ? 'bg-success' : 'bg-info') ?>">
