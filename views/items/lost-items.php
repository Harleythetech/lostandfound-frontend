    ...
    <main class="dashboard-main">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom">
            ...
            <div class="d-flex align-items-center gap-2">
                <a href="<?= APP_URL ?>/notifications" class="btn btn-outline-secondary btn-sm" title="Notifications">
                ...
                <form method="GET" class="row g-3">
                    ...
                    <div class="col-md-2">
                        <select name="status" class="form-select">
                            ...
                            <option value="found" <?= ($filters['status'] ?? '') === 'found' ? 'selected' : '' ?>>Found</option>
                        </select>
                    </div>
                    <div class="col-md-2">
            ...
            <div class="col-md-6 col-lg-4">
                <a href="<?= APP_URL ?>/lost-items/<?= $item['id'] ?>" class="card h-100 border item-card text-decoration-none">
                    <div class="position-relative">
                        ...
                        <img src="<?= API_BASE_URL ?>/<?= htmlspecialchars($primaryImg) ?>" class="card-img-top" style="height: 160px; object-fit: cover;">
                        <?php else: ?>
                        ...
                        <span class="badge <?= getStatusBadgeClass($item['status']) ?> position-absolute top-0 end-0 m-2"><?= ucfirst($item['status']) ?></span>
                        <?php if (!empty($item['reward_offered'])): ?>
                        <span class="badge bg-warning text-dark position-absolute bottom-0 end-0 m-2"><i class="bi bi-gift me-1"></i>₱<?= number_format($item['reward_offered']) ?></span>
                        <?php endif; ?>
                    </div>
                    <div class="card-body p-3">
                        ...
                        <div class="d-flex justify-content-between align-items-center text-muted small">
                            <span><i class="bi bi-geo-alt me-1"></i><?= htmlspecialchars($item['last_seen_location'] ?? 'N/A') ?></span>
        ...
        <div class="text-center py-5">
            ...
            <?php if (isLoggedIn()): ?>
            <div class="mt-3">
                <a href="<?= APP_URL ?>/lost-items/create" class="btn btn-danger"><i class="bi bi-plus-lg me-2"></i>Report Lost Item</a>
            </div>
            <?php endif; ?>
        </div>
        <?php endif; ?>
