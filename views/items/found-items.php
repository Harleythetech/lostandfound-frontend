...
<div class="dashboard-wrapper">
    ...
    <main class="dashboard-main">
            ...
            <div class="col-md-6 col-lg-4">
                <a href="<?= APP_URL ?>/found-items/<?= $item['id'] ?>" class="card h-100 border item-card text-decoration-none">
                    <div class="position-relative">
                        <?php 
                        $img = $item['primary_image'] ?? ($item['images'][0] ?? '');
                        // Handle if it's an object with url property
                        if (is_array($img)) $img = $img['url'] ?? '';
                        // Replace backslashes and normalize path
                        $img = str_replace('\\', '/', $img);
                        $img = preg_replace('#^(/api/|/|api/)#', '', $img);
                        ?>
                        <?php if (!empty($img)): ?>
                        <img src="<?= API_BASE_URL ?>/<?= htmlspecialchars($img) ?>" class="card-img-top" style="height: 160px; object-fit: cover;">
                        <?php else: ?>
                    ...
                    <div class="card-body p-3">
                        ...
                        <div class="d-flex justify-content-between align-items-center text-muted small">
                            <span><i class="bi bi-geo-alt me-1"></i><?= htmlspecialchars($item['found_location'] ?? $item['location_name'] ?? 'N/A') ?></span>
        ...
        <div class="text-center py-5">
            ...
            <?php if (isLoggedIn()): ?>
            <div class="mt-3">
                <a href="<?= APP_URL ?>/found-items/create" class="btn btn-success"><i class="bi bi-plus-lg me-2"></i>Report Found Item</a>
            </div>
            <?php endif; ?>
        </div>
        <?php endif; ?>
