    ...
    <main class="dashboard-main">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-3">
                            <div class="d-flex gap-2">
                                ...
                                <a href="<?= APP_URL ?>/search" class="btn btn-outline-secondary"><i class="bi bi-x-lg"></i></a>
                            </div>
                        </div>
                    </div>
                </form>
        ...
        <div class="row g-3">
            ...
            <?php 
                ...
                $itemImage = $item['primary_image'] ?? ($item['images'][0] ?? '');
                $itemLocation = $item['location_name'] ?? $item['last_seen_location'] ?? 'N/A';
            ?>
            <div class="col-md-6 col-lg-4">
                <a href="<?= APP_URL ?>/<?= $itemType ?>-items/<?= $item['id'] ?>" class="card h-100 border item-card text-decoration-none">
                    <div class="position-relative">
                        <?php if (!empty($itemImage)): ?>
                        <img src="<?= API_BASE_URL ?>/<?= $itemImage ?>" class="card-img-top" style="height: 160px; object-fit: cover;">
                        <?php else: ?>
                        <div class="card-img-top bg-light d-flex align-items-center justify-content-center" style="height: 160px;">
                            <i class="bi bi-image text-muted display-4"></i>
                        </div>
                        <?php endif; ?>
                        <span class="badge <?= $itemType === 'lost' ? 'bg-danger' : 'bg-success' ?> position-absolute top-0 start-0 m-2"><?= ucfirst($itemType) ?></span>
                    </div>
