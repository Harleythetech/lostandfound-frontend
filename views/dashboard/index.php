    ...
    <main class="dashboard-main">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom">
            <div>
                ...
                <p class="text-muted mb-0 small">Welcome back, <?= htmlspecialchars($user['first_name'] ?? 'User') ?>!</p>
            </div>
        ...
        <div class="row g-3 mb-4">
            <div class="col-6 col-lg-3">
                <a href="<?= APP_URL ?>/dashboard/my-lost-items" class="card border-0 shadow-sm h-100 text-decoration-none stat-card-hover">
            ...
            <div class="col-6 col-lg-3">
                <a href="<?= APP_URL ?>/dashboard/my-found-items" class="card border-0 shadow-sm h-100 text-decoration-none stat-card-hover">
                    <div class="card-body py-3">
                        <div class="d-flex align-items-center">
            ...
            <div class="col-6 col-lg-3">
                <a href="<?= APP_URL ?>/dashboard/my-claims" class="card border-0 shadow-sm h-100 text-decoration-none stat-card-hover">
                    <div class="card-body py-3">
                        <div class="d-flex align-items-center">
            ...
            <div class="col-6 col-lg-3">
                <a href="<?= APP_URL ?>/dashboard/my-matches" class="card border-0 shadow-sm h-100 text-decoration-none stat-card-hover">
                    <div class="card-body py-3">
                        <div class="d-flex align-items-center">
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white p-0 border-bottom">
                <ul class="nav nav-tabs nav-fill border-0" id="itemTabs" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link py-3 px-4 <?= $activeTab === 'lost' ? 'active' : '' ?>" 
                                id="lost-tab" data-bs-toggle="tab" data-bs-target="#lost-items" 
                                type="button" role="tab" aria-controls="lost-items" aria-selected="<?= $activeTab === 'lost' ? 'true' : 'false' ?>">
                    ...
                    <li class="nav-item" role="presentation">
                        <button class="nav-link py-3 px-4 <?= $activeTab === 'found' ? 'active' : '' ?>" 
                                id="found-tab" data-bs-toggle="tab" data-bs-target="#found-items" 
                                type="button" role="tab" aria-controls="found-items" aria-selected="<?= $activeTab === 'found' ? 'true' : 'false' ?>">
            ...
            <div class="card-body p-4">
                <div class="tab-content" id="itemTabsContent">
                    <!-- Lost Items Tab -->
                    <div class="tab-pane fade <?= $activeTab === 'lost' ? 'show active' : '' ?>" id="lost-items" role="tabpanel" aria-labelledby="lost-tab">
                        <?php if (!empty($lostItems)): ?>
                        <div class="row g-3">
                            <?php foreach ($lostItems as $item): ?>
                            <div class="col-md-6 col-lg-4">
                                <a href="<?= APP_URL ?>/lost-items/<?= $item['id'] ?>" class="card h-100 border item-card text-decoration-none">
                                    <div class="position-relative">
                                        <?php 
                                        $primaryImg = $item['primary_image'] ?? '';
                                        if (is_array($primaryImg)) $primaryImg = $primaryImg['url'] ?? '';
                                        $primaryImg = str_replace('\\', '/', $primaryImg);
                                        // Remove /api/ prefix if present, then add API_BASE_URL
                                        $primaryImg = preg_replace('#^(/api/|/|api/)#', '', $primaryImg);
                                        ?>
                                        <?php if (!empty($primaryImg)): ?>
                                        <img src="<?= API_BASE_URL ?>/<?= htmlspecialchars($primaryImg) ?>" class="card-img-top item-card-img" alt="<?= htmlspecialchars($item['title'] ?? '') ?>">
                                        <?php else: ?>
                                        <div class="card-img-top item-card-img bg-light d-flex align-items-center justify-content-center">
                                        ...
                                        <span class="badge bg-warning text-dark position-absolute top-0 end-0 m-2">
                                            <i class="bi bi-gift me-1"></i>₱<?= number_format($item['reward_offered']) ?>
                                        </span>
                                        <?php endif; ?>
                                    </div>
                                    <div class="card-body p-3">
                                        <h6 class="card-title mb-1 text-truncate text-dark"><?= htmlspecialchars($item['title'] ?? '') ?></h6>
                                        <p class="card-text small text-muted mb-2 text-truncate"><?= htmlspecialchars($item['description'] ?? '') ?></p>
                                        <div class="d-flex justify-content-between align-items-center">
                                            <small class="text-muted">
                                                <i class="bi bi-geo-alt me-1"></i><?= htmlspecialchars($item['last_seen_location'] ?? 'N/A') ?>
                                            </small>
                                            <small class="text-muted"><?= formatDate($item['last_seen_date'] ?? $item['created_at'] ?? '') ?></small>
                                        </div>
                                    </div>
                                </a>
                            </div>
                    ...
                    <!-- Found Items Tab -->
                    <div class="tab-pane fade <?= $activeTab === 'found' ? 'show active' : '' ?>" id="found-items" role="tabpanel" aria-labelledby="found-tab">
                        <?php if (!empty($foundItems)): ?>
                        <div class="row g-3">
                            <?php foreach ($foundItems as $item): ?>
                            <div class="col-md-6 col-lg-4">
                                <a href="<?= APP_URL ?>/found-items/<?= $item['id'] ?>" class="card h-100 border item-card text-decoration-none">
                                    <div class="position-relative">
                                        <?php 
                                        $primaryImg = $item['primary_image'] ?? '';
                                        if (is_array($primaryImg)) $primaryImg = $primaryImg['url'] ?? '';
                                    ...
                                    <div class="card-body p-3">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            <small class="text-muted">
                                                <i class="bi bi-geo-alt me-1"></i><?= htmlspecialchars($item['found_location'] ?? 'N/A') ?>
                                            </small>
                                            <small class="text-muted"><?= formatDate($item['found_date'] ?? $item['created_at'] ?? '') ?></small>
                                        </div>
                                    </div>
                                </a>
                            </div>
