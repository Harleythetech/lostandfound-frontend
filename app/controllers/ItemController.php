...
class ItemController {
    ...
    public function lostItems() {
        ...
        if ($category_id) $queryParams[] = "category_id=" . urlencode($category_id);
        if ($location_id) $queryParams[] = "location_id=" . urlencode($location_id);
        ...
        if (isset($categories['success'])) $categories = [];
    ...
    public function storeLostItem() {
        ...
        if (empty($data['last_seen_time'])) unset($data['last_seen_time']);
        if (empty($data['unique_identifiers'])) unset($data['unique_identifiers']);
        if ($data['reward_offered'] === null) unset($data['reward_offered']);
        if (empty($data['email'])) unset($data['email']);
        if (empty($data['phone_number'])) unset($data['phone_number']);
        ...
        if ($response['status'] === 201 || $response['status'] === 200) {
            ...
            if (isset($response['data']['errors']) && is_array($response['data']['errors'])) {
                $errors = array_map(fn($e) => is_array($e) ? ($e['message'] ?? json_encode($e)) : $e, $response['data']['errors']);
                $message .= ': ' . implode(', ', $errors);
            }
            setFlash('danger', $message);
            redirect('/lost-items/create');
        }
    }
    ...
    public function foundItems() {
        ...
        $locResponse = apiRequest('/locations?active_only=true', 'GET');
        $locations = $locResponse['data']['data'] ?? $locResponse['data'] ?? [];
        if (isset($locations['success'])) $locations = [];
    ...
    public function storeFoundItem() {
        ...
        // Remove null/empty optional fields
        if (empty($data['found_time'])) unset($data['found_time']);
        if ($data['storage_location_id'] === null) unset($data['storage_location_id']);
        if (empty($data['storage_notes'])) unset($data['storage_notes']);
        if (empty($data['unique_identifiers'])) unset($data['unique_identifiers']);
        if (empty($data['condition_notes'])) unset($data['condition_notes']);
